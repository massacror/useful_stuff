...
#+begin_src bash
cp /home/kim/org-roam/20230715112804-ma_config_emacs.org /home/kim/Documents/useful_stuff_depot/config_emacs.org
...
** [#A] HELM config
:PROPERTIES:
:ID:       32d4cc31-7231-46ff-9486-f4139488be7d
:END:
...
#+end_src
*** problème helm bloqué history
:PROPERTIES:
:ID:       9a1b3124-ed4c-4c8f-babf-df50c1ba4be1
:END:

Faut utiliser helm-next-source : *C-o* et *M-o*
...
#+begin_src elisp :tangle ~/.emacs.d/config-perso.el
    ...
    (use-package evil
    :ensure t
    :init (setq evil-want-C-i-jump nil);; permet d'avoir tab 
    )
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/config-perso.el
    ...
    (defun bash-run ()
      "Run the current file in a terminal."
      (interactive)
      (let ((proc-name "bash-run"))
	(when (get-buffer proc-name)
	  (kill-buffer proc-name))
	(start-process proc-name proc-name "gnome-terminal" "-e" (concat (buffer-file-name) " && read -n 1 -s -r -p \"Press any key to continue\""))))

    (add-hook 'sh-mode-hook
	      (lambda () (local-set-key (kbd "C-c C-c") 'bash-run))
	      )
#+end_src
...
[[/home/kim/org-roam/reveal_presentations]]
** config elisp
#+begin_src emacs-lisp :tangle ~/.emacs.d/config-perso.el
;; Reveal.js + Org mode
(use-package ox-reveal :ensure t)
(setq Org-Reveal-root "file:///home/kim/org-roam/reveal_presentations/reveal.js/")
...
* org roam data base
#+begin_src emacs-lisp :tangle  ~/.emacs.d/config-perso.el
(setq org-roam-db-location (file-truename "~/org-roam/roamDatabaseLinux.db"))
#+end_src
...
[[file:~/.emacs::'(:foreground default :background default :scale 2 :html-foreground "Black" :html-background "Transparent" :html-scale 1.0 :matchers][.emacs DELL]]
...
;; On set la commmande de compilation par défaut :
(setq compile-command "g++ -g -pedantic -Wall -c")
...
#+RESULTS:
| (lambda nil (local-set-key (kbd C-c C-v C-t) 'my-org-babel-tangle-chmod)) | (lambda nil (setq evil-auto-indent nil)) | (lambda nil (local-set-key (kbd  n p) 'org-id-get-create) (local-set-key (kbd  n m) 'org-roam-refile)) | my-org-mode-hook | (lambda nil (local-set-key (kbd  n g) 'org-roam-ui-change-local-graph) (local-set-key (kbd  n d) 'org-roam-ui-remove-from-local-graph) (local-set-key (kbd  n a) 'org-roam-ui-add-to-local-graph) (local-set-key (kbd  n z) 'org-roam-ui-node-zoom)) | (lambda nil (local-set-key (kbd <tab>) 'org-cycle)) | copilot-mode | #[0 \301\211\207 [imenu-create-index-function org-imenu-get-tree] 2] | (lambda nil (org-bullets-mode 1)) | undo-tree-mode | (lambda nil (local-set-key (kbd  i) 'org-download-clipboard)) | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-fold-show-all append local] 5] | #[0 \300\301\302\303\304$\207 [add-hook change-major-mode-hook org-babel-show-result-all append local] 5] | org-babel-result-hide-spec | org-babel-hide-all-hashes |
...
*** load dans emacs
En fait faut juste choper copilot.el et l'executer quoi
#+begin_src emacs-lisp :tangle ~/.emacs.d/config-perso-dell.el

(load-file "~/copilot-emacsd/copilot.el/copilot.el")


(if (daemonp)
    (add-hook 'after-make-frame-functions
		(lambda (frame)
		(setenv "PATH" (concat "/home/kim/bin/node-v20.11.0-linux-x64/bin" (getenv "PATH")))
		(setq exec-path (append '("/home/kim/bin/node-v20.11.0-linux-x64/bin") exec-path)))))
...
(add-hook 'org-mode-hook rk/copilot-enable-for-org)
#+end_src
*** copilot old
    #+begin_src emacs-lisp
:tangle ~/.emacs.d/config-perso-dell.el
	  ;; A SUPPRIMER POUR CEUX QUI UTILISENT MA CONFIG:
	  ;; Permet d'avoir github copilot dans emacs:
	  ;; Source : https://github.com/zerolfx/copilot.el
	  ;; Pour trouver le serveur node :
